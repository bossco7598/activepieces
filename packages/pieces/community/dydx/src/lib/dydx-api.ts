...
export const DYDX_PROTOCOL_SLUG = 'dydx';
export const DYDX_COINGECKO_ID = 'dydx-chain';
...
export async function getProtocolData(): Promise<DefiLlamaProtocol> {
  const response = await httpClient.sendRequest<DefiLlamaProtocol>({
    method: HttpMethod.GET, url: `${DEFILLAMA_BASE_URL}/protocol/${DYDX_PROTOCOL_SLUG}`,
  });
  return response.body;
}

export async function getDydxTokenData(): Promise<CoinGeckoPrice> {
  const response = await httpClient.sendRequest<CoinGeckoPrice>({
    method: HttpMethod.GET, url: `${COINGECKO_BASE_URL}/coins/${DYDX_COINGECKO_ID}`,
